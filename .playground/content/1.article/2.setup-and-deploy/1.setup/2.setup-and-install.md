...
#### Ignore this
If you see some warnings in the terminal, about _"glob patterns"_, just ignore them.[^2]  
![Picture of globpatterns-warning](./globpatterns-warning.webp){.border-solid .border-4 width="400"}
...
Please install bubblewrap globally (with the `-g`-word), if you want to create more than this app.[^3]
```bash
# Installing bubblewrap
...
[^2]: It's only a warning in developer-mode, and do no harm in production. _Vite-plugin-pwa_ creates this error. They hasn't found the resons to this warnings, so we have to live with them even if we don't like them. 

[^3]: Remember you get lifetime access to _Google Play Console_ for 25 dollar, and think about your first app as a learning-project. 
